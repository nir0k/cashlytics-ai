...
gaps:
  - truth: "TypeScript compiles cleanly"
    status: failed
    reason: "TypeScript compilation fails due to orphaned file src/lib/actions/account-actions.ts which was not refactored and is missing userId in insert"
    artifacts:
      - path: "src/lib/actions/account-actions.ts"
        issue: "Missing requireAuth guard and userId filtering; causes TS2769 error on line 63 - 'Property userId is missing in type'"
    missing:
      - "Either refactor src/lib/actions/account-actions.ts with requireAuth and userId filtering"
      - "Or delete src/lib/actions/account-actions.ts (it is not imported anywhere - orphaned code)"
---
...
| 5   | TypeScript compiles cleanly                                        | ✗ FAILED   | TS2769 error in src/lib/actions/account-actions.ts - missing userId property                   |
...
| `src/lib/actions/account-actions.ts` | Legacy account actions               | ✗ ORPHANED | NOT refactored, no auth, causes TypeScript error          |
...
| ---------------------------------- | ---- | ------------- | ---------- | ------------------------------------------------------------------------------------------ |
| src/lib/actions/account-actions.ts | -    | No auth guard | 🛑 Blocker | Orphaned file causes TypeScript error, blocks compilation                                  |
| src/actions/dashboard-actions.ts   | 147  | TODO comment  | ℹ️ Info    | "incomeTrend: 0 // TODO: Historische Daten vergleichen" - incomplete feature, not blocking |
...
**One critical gap blocks goal achievement:**

1. **TypeScript compilation failure** — The file `src/lib/actions/account-actions.ts` was not refactored and causes a TypeScript error:
   - Error: `TS2769: Property 'userId' is missing in type` at line 63
   - This file has NO requireAuth guard and NO userId filtering
   - It was NOT included in any of the 5 PLAN files (all plans only targeted `src/actions/*.ts`)
   - It is NOT imported anywhere in the codebase (orphaned code)

**Resolution options:**

- **Option A (Recommended):** Delete `src/lib/actions/account-actions.ts` — it's orphaned and duplicate of `src/actions/account-actions.ts` which is properly refactored and used
- **Option B:** Refactor the file with requireAuth and userId filtering to match the pattern
